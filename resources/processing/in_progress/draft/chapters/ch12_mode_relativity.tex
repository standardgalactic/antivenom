...
static cues (Chapter~\ref{ch:painting}), $r_{c,t}$ is the viewer's
percept of the canvas rather than the canvas itself, and how close that
percept sits to the canvas is exactly the question
Chapter~\ref{ch:mem8}'s fork leaves open (\cref{oq:mem8-fork}). A contract
clause is high-fidelity for the task of reading its literal text; the
same contract is low-fidelity for the task of establishing original
intent, which is typically litigated by reconstructing a trace-based
...
Painting & Causal inference & Viewer's percept of the canvas & Open (\cref{oq:mem8-fork}) \\
