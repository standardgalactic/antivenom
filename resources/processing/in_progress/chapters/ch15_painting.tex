...
\begin{remark}[This Conclusion Does Not Depend on Chapter~\ref{ch:mem8}'s Fork]
\label{rem:painting-fork-independent}
This chapter's argument concerns the rendering from event to canvas: the
painter's projection, which is low-fidelity for causal-inference tasks
by \cref{prop:open-causal-space} regardless of who later views the
canvas or how directly they perceive it. A further rendering step --
from canvas to a specific viewer's percept -- is where
Chapter~\ref{ch:mem8}'s unresolved fork actually enters
(\cref{oq:mem8-fork}), and Chapter~\ref{ch:mode-relativity} treats it
separately for exactly this reason. Because fidelity cannot increase
under further rendering (\cref{prop:admissibility-monotone}),
\cref{prop:benign-ambiguity}'s conclusion is unaffected by how that
further question is eventually resolved.
\end{remark}
