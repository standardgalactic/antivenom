...
\subsection{Why Local Rationality Is Not Global Rationality}
\label{sec:local-not-global}

It is worth pausing on a distinction \cref{def:budget-relay} makes
available but does not itself name: a stage can be perfectly rational
relative to everything it can see, and the chain it belongs to can still
be irrational relative to everything no single stage could see. This is
not a paradox and not a failure of individual reasoning. Each stage's
optimization problem is well-posed and correctly solved: given $A_i$,
$\tau_i$, and $\beta_i$, fund what $\tau_i$ requires as well as the
budget allows. Nothing about that problem asks the stage to consider
$\tau_{i+1}$, let alone $\tau^*$, the eventually-relevant task
Chapter~\ref{ch:composition-failure} constructs. Local rationality is
rationality relative to a local objective function; global rationality,
if the phrase means anything at all here, would have to be rationality
relative to every future task the chain might eventually serve --- and
\cref{prop:task-unavailability} already showed that objective is not
well-posed at any single stage, since the relevant future tasks are not
yet known when that stage acts. The chain cannot be irrational for
failing an optimization problem it was never given, and it cannot be
rational for solving one either; ``global rationality'' is simply not
an available property for any individual stage to have or lack. What
survives is only the composition-level question Chapter~\ref{ch:composition-failure}
asks: given that every stage solved its own well-posed local problem, was
the sequence of solutions jointly sufficient for a task none of them
was solving for? The answer can be no, and that is the entire content of
this monograph's central theorem --- not a claim that someone reasoned
badly, but a claim that a well-posed collection of local optima need not
...
\cref{rem:root-recovery-persistence-set} (Chapter~\ref{ch:four-operations})
situates this proposition within the more general persistence-set apparatus
of Persistence Before Truth, and reframes the bootstrapping problem below in
terms of the measure of a specific persistence set rather than only
qualitatively.
...
\subsection{The Loss Ledger as Institutional Memory}
\label{sec:ledger-as-witness}

The loss ledger is worth a second look through the vocabulary
\S\ref{sec:witnesses} reported, at its own weaker tier, from The
Geometry of Witnesses. That source's reported central question --- who
maintains the repair operator, once repair operators themselves can be
lost --- has a direct analogue here: who maintains the loss ledger, once
ledgers themselves can be discarded, corrupted, or never consulted?
$L_i$ is not self-maintaining. It is itself a rendering
(\cref{def:rendering}, applied to the ledger rather than to $A_i$), with
its own discard set, and an institution that stops funding its own
record-keeping loses the ledger before it loses the substance the ledger
was tracking. In the reported vocabulary of \S\ref{sec:witnesses}, a
well-archived loss ledger --- copied across independent systems, cited
in multiple downstream audits, backed up in more than one location --- has
high reconstruction redundancy $\rho_W$; a ledger kept in one place by one
administrator, discarded the moment that role changes hands, has $\rho_W =
1$ and is one retirement away from $\rho_W = 0$. This is not a new
mechanism this monograph is adopting, since \S\ref{sec:witnesses}'s
apparatus remains unverified; it is a naming of a real, practical
vulnerability that the loss ledger, as defined here, does nothing to
prevent on its own -- a ledger is only as durable as whatever maintains
it, and \cref{def:loss-ledger} is silent on what that is.
