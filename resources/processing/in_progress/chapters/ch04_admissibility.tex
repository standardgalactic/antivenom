...
\subsection{Tasks as Filters on Distinction Space}

It helps to have a single image in mind before the formal machinery
arrives. A task does not need everything a substrate might carry; it
needs a specific subset of the available distinctions, and is
indifferent to the rest. Verifying a proof needs the logical content and
nothing about the author's handwriting; reading a map for a driving
route needs road connectivity and is indifferent to elevation contours a
hiking map would need instead. A task, in this sense, acts as a filter
over distinction space: it partitions what a substrate carries into
what matters for this purpose and what does not, and
Chapter~\ref{ch:distinctions}'s benign/corrupting distinction
(\cref{def:benign-loss,def:corrupting-loss}) is already implicitly using
this filter, since ``not needed for the task'' is exactly ``outside the
filter.'' The rest of this chapter makes the filter precise enough to
compute with, but the filter is the idea; the correctness set below is
...
no role in that determination. By \cref{prop:admissibility-upper-bound},
$\admis{\tau}{r} \subseteq \admis{\tau}{e}$ always, and whenever $e$
itself is sufficient for $\tau$, every continuation in $\admis{\tau}{e}$
already lies in $\mathcal{C}_\tau(e)$; combining the two containments
gives $\admis{\tau}{r} \subseteq \mathcal{C}_\tau(e)$, which is
