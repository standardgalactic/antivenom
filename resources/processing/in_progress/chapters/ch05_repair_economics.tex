...
\begin{remark}[Repair Cost as Deficit-Closing Investment]
\label{rem:repair-cost-as-deficit}
Chapter~\ref{ch:four-operations} imports Distinguishability Geometry's
coding deficit $\delta_T(x)$ (\cref{def:coding-deficit}), and the connection
to repair cost is close enough to be worth stating even before that chapter
is reached: funding $\repcost{d}$ is, in the vocabulary of that later
chapter, precisely the investment required to keep $d$'s minimal pair
distinguishable under an ontology $T$ that would otherwise identify them --
that is, to hold $\delta_T(d)$ at zero rather than letting it rise. This
monograph's repair-economic vocabulary and Distinguishability Geometry's
deficit vocabulary are describing the same underlying phenomenon from two
directions: one asks what it costs to prevent a collapse, the other asks
how much description length is lost once the collapse has already
happened.
\end{remark}
...
This definition is stated exactly as \emph{Conservation of Ambiguity}
gives it, correctly attributed. It is worth flagging, rather than
discovering by accident later, that the same source text attributes this
formula to Observability Is Restorability, and Observability Is
Restorability's own verified text defines repair interference differently
-- algebraically rather than as a derivative.
\cref{rem:interference-inconsistency} (Chapter~\ref{ch:four-operations})
gives the precise comparison; nothing in this chapter or those that follow
depends on resolving it, since this monograph uses only the derivative
form, consistently, throughout.
