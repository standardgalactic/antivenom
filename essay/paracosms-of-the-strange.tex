...
\section{A Fourth Foundation --- Contestability and the Sealed Circle}

A further case complicates the taxonomy in a direction none of the first six touch: Robert Graves. A severely wounded survivor of a catastrophic war, and afterward a disciplined historical novelist who reconstructed the past by reading official testimony against its own grain, treating the surviving record of the marginalized and dismissed as more diagnostic than the record of institutional authority, Graves eventually turned that same suspicion on the historical record as a whole \cite{graves1929}. The claim that followed, in \emph{The White Goddess}, was that a coherent religious-poetic system, centered on a figure and a set of ritual relations, survived nowhere as a single document but everywhere as scattered correspondence: names, calendars, images, and fragments of ritual language, recoverable only by someone sufficiently attuned to the pattern connecting them \cite{graves1948}. This is not, on its face, an unreasonable proposal. Distributed survival across degraded and incomplete carriers is a real property of cultural transmission, and every case examined in this essay depends on some version of exactly that possibility. What distinguishes Graves's case from the six already examined is not the ambition of the reconstruction but what happened to disagreement with it.

Scholarly objection to the reconstruction was treated not as evidence bearing on it but as evidence of the objector's own limitation: someone too bound to ordinary prose reading to apprehend an order that operated by different rules. Once that move is available, every disconfirming case becomes, instead, a confirming case for the critic's inadequacy, and the channel by which an archive is checked against anything outside itself closes without anyone needing to close it deliberately. The clearest practical cost of this closure appeared in the translation of the Rubaiyat that Graves produced with Omar Ali-Shah, offered as authentic ancient material transmitted through a family manuscript unavailable to ordinary scholarship \cite{gravesalishah1967}. Graves accepted and publicly defended that provenance claim; when specialists challenged it, the privileged transmission on which the project depended proved unable to withstand ordinary evidential scrutiny, in a way the provenance checks applied without hesitation to the historical fiction, where invented detail was always understood as invention, would have caught immediately had they been applied here. They were not applied, because the reconstructive framework had already pre-classified skepticism as a symptom of the skeptic rather than a legitimate check on the claim.

The same pattern reappeared, in a different register, in Graves's own working life. The suspicion of institutional command that had driven an earlier, explicit refusal, a vow never again to operate under anyone's orders \cite{graves1929}, did not disappear; it relocated. It would overstate the case to say command was simply reconstituted under Graves's own management: interpretive authority over the reconstructed system came to be organized around a circle centered on Laura Riding, in which exceptional poetic insight, personal devotion, obedience, and increasingly insulated authority became entangled, and Graves was at times as much subject to that authority as source of it \cite{seymour1995,rgraves1995}. What survives the correction is narrower than the original claim but still real: Graves's flight from institutional command did not eliminate relations of command from his intellectual life, it only moved them somewhere less accountable.

What Graves's case, call it the contestability case, adds to the taxonomy is a term the other six hold fixed without noticing they are holding it fixed: a function governing how the system responds when challenge arrives from outside its own interpretive frame. In every case examined so far, whatever else varies, a legitimate external challenge is at least an available input, one that could in principle revise the archive, the interpreter's own contribution, the evidential weighting, or the constructed world, even where nothing in the original taxonomy modeled how that input was used. Here that channel is not merely weak or attenuated in the way Guyon's discipline attenuates the interpreter's own carryover; it is inverted. Challenge is not ignored. It is metabolized, converted into further material confirming the very framework it was meant to test, so that resistance to correction becomes, self-servingly, additional evidence for the correctness of the position resisting correction. This is not the same failure as Swedenborg's fixed-point convergence. Swedenborg's categories stabilize under the ordinary pressure of accumulating material, absorbing new evidence and disconfirming evidence alike into an already-coherent scheme; nothing in that mechanism treats a challenge differently from any other new arrival, which is why Appendix E's identifiability problem, not a captured correction channel, is the right diagnosis of his case. Graves's mechanism is narrower and more specific: challenge in particular, and only challenge, is routed differently than material that simply confirms, so that disagreement becomes special evidence in a way agreement never has to be. It is a specific and separable failure of the correction channel itself, and Appendix G extends the formal apparatus to give it a name.

\section{What the Archive Is Doing}


The seven cases can now be pulled back from their differences toward the single claim the essay has been assembling all along, but that claim has to be broader than the one available after only three cases, because Guyon, Payne-Gaposchkin, Le Guin, and the contestability case (Section 8) each show that the archive is not, by itself, the fundamental object. What is fundamental is a coupled system: an accumulating record, an interpreter whose own prior state feeds into what comes next, a source of constraint that need not originate within the record, a surrounding space of possibility within which interpretation occurs at all, and a channel by which challenge from outside the system can or cannot reach it. The archive, as examined in the first three cases, is only one component of this system, the component that varies while the other four are held fixed. Once that is visible, the earlier claim can be restated more precisely rather than abandoned.

The operation examined throughout can be written as a single update,
\[
x_{n+1} = \rho\!\left(\alpha_n x_n,\, A_n,\, E_n,\, W_n,\, c_n\right),
\]
where $A_n$ is the accumulated archive examined in the first three cases, $E_n$ is constraint supplied independently of that archive, $W_n$ is the constructed space of possibility within which interpretation occurs, $\alpha_n$ governs how strongly the interpreter's own prior state is permitted to condition the next one, and $c_n$ governs how external challenge is routed once it arrives. Luhmann, Swedenborg, and Dick each hold $\alpha_n$, $E_n$, $W_n$, and $c_n$ fixed at their default values and vary only $A_n$: Luhmann exploits it to maximize divergence, Swedenborg lets it converge to a stable regime, and Dick lets it act on an endogenous object that the archive itself keeps redefining. The three foundational cases of Section 7 each vary one of the remaining terms instead. Guyon drives $\alpha_n$ toward zero, attenuating the interpreter's own contribution rather than accumulating, stabilizing, or recursively amplifying it. Payne-Gaposchkin gives $E_n$ unusual authority, subordinating the archive's internal coherence to a body of evidence that does not originate within it. Le Guin varies $W_n$ directly, constructing the very space of constraint within which an archive's interpretive positions are generated, which is why her case looks least like an archive at all: she is manipulating a term the other six cases treat as simply given. The contestability case (Section 8) varies $c_n$: rather than weakening the correction channel, as Guyon weakens $\alpha_n$, it inverts it, routing external challenge back into $A_n$ as confirming material instead of into $E_n$ as revising material.

This reframing changes what it means to say the earlier claim holds across all seven cases. An archive, on this account, does not primarily solve a storage problem; storage would be satisfied by any durable medium, and a locked drawer stores as reliably as a Zettelkasten. What all seven systems share is that each solves a problem in this larger coupled space rather than a merely spatial one, making it possible for a later interpretive state to encounter earlier material, external constraint, an alternate constructed world, or an external challenge, under conditions that did not exist when that material, constraint, world, or challenge first arrived. Preservation is not the operative concept in any of the seven cases. Retrieval, whether of an inscription, an independent measurement, a deliberately constructed alternative society, or a contested claim, is never mere recovery; $\rho$ does not return its inputs unchanged, it produces a new state from their encounter.

The three original cases can now be seen as differing only in how $A_n$ behaves while $\alpha_n$, $E_n$, $W_n$, and $c_n$ sit at whatever value is typical of ordinary interpretive practice: full carryover of the prior state, no privileged external check, a single given world, and a correction channel the original taxonomy simply did not model, rather than one that can safely be assumed well-behaved by default. Luhmann's non-hierarchical addressing maximizes the range of later interpretive positions reachable from any given card, and the resulting non-convergence is what a system optimized for that purpose should produce. Swedenborg's cross-referencing, applied to material with a cosmological stake in coherence, converges because the positions it generates are constrained to remain compatible with an accumulating shared scheme, though whether that convergence already partially internalizes challenge, rather than routing it cleanly into $E_n$, is exactly the kind of question the unmodeled channel leaves open. Dick's \emph{Exegesis} generates positions that feed back into the identity of the event they interpret, which is why neither convergence nor open divergence describes it. The four foundational cases show that none of $\alpha_n$, $E_n$, $W_n$, or $c_n$ needs to sit at its default. Guyon shows the interpreter's own carryover can be suppressed rather than exploited. Payne-Gaposchkin shows the archive can be made to answer to a constraint it did not generate, with the further complication that social authority about evidence is not the same thing as the evidence itself. Le Guin shows the surrounding world can be built rather than found, turning the constraint-space itself into an experimental variable. The contestability case does not merely select another setting for a variable the first six cases already exhibited; it forces the variable to be introduced at all, since nothing in the original taxonomy modeled $c_n$ in the first place. It shows the correction channel itself can be captured, so that the very mechanism meant to check the system against anything outside it is repurposed to insulate the system from exactly that check.

There is a further consequence worth drawing out before the coda, concerning what this fuller account implies about the relationship between a builder and the system they build. Across all seven cases, whether the varied term is the archive, the interpreter's own contribution, the source of external constraint, the surrounding world, or the correction channel, the builder is not simply managing an overflow of material that a better memory would have handled directly. They are constructing, attenuating, subordinating to, capturing, or fabricating one term in a coupled dynamical system whose behavior could not have been fully anticipated at the moment the system was first set in motion. This is what licenses treating Luhmann's ordinary scholarly archive, Swedenborg's and Dick's extraordinary ones, Guyon's disciplined withdrawal from elaboration, Payne-Gaposchkin's evidentially answerable dataset, Le Guin's deliberately constructed societies, and the contestability case's captured correction channel under a single description. None of the seven is generative, or, in the contestability case, corrosive, because of the extraordinariness of its content. Each is generative or corrosive because it manipulates, in a different way, one term of the same underlying coupled system, and something new, or something newly sealed off, is produced by that manipulation that no single term, held fixed, could have produced alone.
...
\section{The Generalized Model: Attenuation, External Constraint, Constructed Worlds, and Contestability}

The dynamics developed in Appendices A through F treat $A_n$, the archive, as the only term subject to variation, with the interpreter's carryover, the source of constraint, the surrounding space of possibility, and the correction channel all held at implicit default values. Sections 7 and 8 argued informally that four further cases, Guyon, Payne-Gaposchkin, Le Guin, and Graves, are best understood as varying exactly these other terms. This appendix states that generalization precisely.
...
\]
where $A_n$ retains its meaning from Appendix A, $E_n$ is a constraint process supplied independently of $A_n$, $W_n$ is a constructed constraint-space or world in which interpretation occurs, $\alpha_n \in [0,1]$ is a scalar or operator governing the degree to which the prior interpretive state $x_n$ is carried forward into the encounter that produces $x_{n+1}$, and $c_n$ is a routing function governing where a challenge arriving from outside the system at step $n$ is sent. The three-way taxonomy of Appendix C is recovered as the special case in which $\alpha_n \equiv 1$, $E_n$ exerts no independent authority over the update, $W_n$ is held fixed at a single default world $W^\ast$ throughout, and $c_n$ routes any challenge into $E_n$ by default. Luhmann, Swedenborg, and Dick differ from one another only in the behavior of $A_n$ under that restriction.
...
The contestability case of Section 8 requires the term with no analogue anywhere in Appendices A through F or in the three cases above: a routing function $c_n$ governing what happens to a challenge $\chi_n$ that originates outside the system at step $n$. In the well-behaved regime implicit everywhere else in this appendix, $c_n$ sends $\chi_n$ into $E_n$, so that a legitimate external challenge is at least available to revise the update in the manner formalized for Payne-Gaposchkin's case:
\[
c_n(\chi_n) \in E_{n+1}, \qquad \text{well-behaved regime.}
\]
The captured regime is the one in which $c_n$ instead sends $\chi_n$ into $A_n$:
\[
c_n(\chi_n) \in A_{n+1}, \qquad \text{captured regime,}
\]
so that a challenge intended to test the archive against something outside it is absorbed as further content confirming the archive, indistinguishable at the next step from any other entry the archive generated on its own. This is not the same operation as setting $\eta = 0$ in Payne-Gaposchkin's identifiability model, which merely fails to weight external evidence; the captured regime actively converts external evidence into internal support, so that the observed frequency of external challenge, rather than disconfirming the archive, appears within it as evidence of persecution, misunderstanding, or the challenger's own inadequacy. Where Guyon's $\alpha_n \to 0$ suppresses the interpreter's carryover and Payne-Gaposchkin's $\eta > 0$ strengthens the archive's answerability to $E_n$, the captured regime of $c_n$ does something categorically different from either: it does not merely fail to check the archive against the outside, it repurposes the outside as a further, self-confirming interior.

This is also the precise point at which Graves's case must be distinguished from Swedenborg's, since both, described loosely, involve an archive insulated from external correction. The distinction is that Swedenborg's convergence is a property of $A_n$ alone. Write the stabilization informally as a map $\sigma$ applied uniformly to whatever material arrives,
\[
A_{n+1} = \sigma(A_n, m_n),
\]
where $m_n$ is any new material presented at step $n$, without distinguishing disconfirming material from confirming material; $\sigma$ folds both into the accumulating scheme by the same operation, because the scheme was built to be comprehensive rather than to be checked. Nothing in $\sigma$ treats a challenge $\chi_n$ differently from an ordinary new observation, which is why Swedenborg's case belongs to the identifiability problem of Appendix E, a failure to distinguish observationally equivalent hypotheses, rather than to a captured $c_n$. Graves's mechanism is narrower and more specific than uniform absorption: it requires $c_n$ to treat $\chi_n$ differently precisely because it is a challenge,
\[
c_n(\chi_n) \in A_{n+1}
\quad\text{while}\quad
c_n(m_n) \in E_{n+1} \text{ or } A_{n+1} \text{ as ordinary material would dictate,}
\]
so that disagreement specifically, and not new information in general, becomes special evidence. A system can therefore exhibit $\sigma$-style convergence with $c_n$ left entirely unmodeled, as the original taxonomy does for Swedenborg; only a system that actively discriminates challenge from non-challenge, routing the former into confirmation while leaving the latter to ordinary processing, exhibits the captured regime this appendix formalizes.

\subsection*{The General Case}

Stated together, the seven cases occupy seven distinct regions of the same five-term model. Luhmann varies $A_n$ to maximize divergence. Swedenborg varies $A_n$ toward a fixed point. Dick lets $A_n$ act recursively on an endogenous object $o_n$, as in Appendix C. Guyon drives $\alpha_n$ toward zero. Payne-Gaposchkin raises $\eta$, the effective weight on $E_n$. Le Guin varies $W_n$ directly, generating divergent trajectories under a shared archive and a shared interpreter. The contestability case sends $c_n$ into the captured regime, routing $\chi_n$ into $A_{n+1}$ rather than $E_{n+1}$. No one of these seven operations is reducible to any other, and the taxonomy of Appendix C, restricted as it was to variation in $A_n$ alone, could not in principle have distinguished them. The archive, on this fuller account, is one adjustable term in a coupled system of interpreter, evidence, world, and contestability, not the system itself.
...
\bibitem{graves1929}
Robert Graves,
\emph{Good-Bye to All That: An Autobiography},
Jonathan Cape, London, 1929;
rev.\ ed., Cassell, London, 1957.

\bibitem{graves1948}
Robert Graves,
\emph{The White Goddess: A Historical Grammar of Poetic Myth},
Faber \& Faber, London, 1948;
rev.\ ed., 1961.

\bibitem{gravesalishah1967}
Robert Graves and Omar Ali-Shah,
\emph{The Rubaiyyat of Omar Khayyam},
Cassell, London, 1967.

\bibitem{seymour1995}
Miranda Seymour,
\emph{Robert Graves: Life on the Edge},
Doubleday, London, 1995.

\bibitem{rgraves1995}
Richard Perceval Graves,
\emph{Robert Graves and the White Goddess: The White Goddess, 1940--1985},
